...
- **`scheduler`**: Event-driven + dynamic-wakeup planner (single instance, see `docs/developer_guide/SERVER_COMPONENTS.md`) — backs up a host as soon as it registers online (via a durable Redis Stream, not Pub/Sub, so a scheduler restart or reconnect never misses the event), otherwise sleeps until the next real deadline across hosts, archive profiles, and nightly maintenance (each tracked against its own persisted "last done" state, no Apalis cron involved). No periodic safety-net ceiling: a due host with no entry (or an offline one) in the resolver cache is excluded from the wakeup computation entirely (it waits on its own online event, not a timer) rather than being polled on a fixed backoff — a resolver entry expires on its own shortly after a self-registering host actually goes offline, so "no entry" is the common case, not just an edge case. A host configured with `noOnlineDetection: true` (fixed-IP hosts with no self-registration, e.g. a separate/unreachable network) is exempt from that exclusion and keeps the previous behavior — retried on a short backoff until found. A config change (new host, re-activated schedule, edited archive profile/nightly cron) is only picked up on scheduler restart. Honors per-host/global blackout windows.
