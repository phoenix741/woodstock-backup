      ...
      <v-container>
        <v-row no-gutters>
          <v-col cols="8">
            <span class="font-weight-light">
              <v-icon small>mdi-cloud-download</v-icon> {{ task.name }} - {{ startDate }} - {{ task.percent }}%
            </span>
          </v-col>
        ...
        <v-row class="pt-5" no-gutters>
          <v-col cols="12" v-if="task.state == 'RUNNING'">
            <v-progress-linear v-model="task.percent" :indeterminate="!task.percent"></v-progress-linear>
          </v-col>
        </v-row>
      </v-container>
    </v-expansion-panel-header>
    <v-expansion-panel-content>
      <v-list>
        <template v-for="(subtask, index) in task.subtasks">
          ...
          <v-divider :key="subtask.name + '-divider'" v-if="index !== task.subtasks.length - 1"></v-divider>
        </template>
      </v-list>
    </v-expansion-panel-content>
  </v-expansion-panel>
...
export default class Task extends Vue {
  ...
  get progressText() {
    return `${humanize.filesize(this.task.speed)}/s - ${this.task.newFileCount} ${
