...
* **Scheduler (`scheduler`)**: Event-driven + dynamic-wakeup planner (single instance). Backs up a host as soon as it comes back online (a durable Redis Stream, not Pub/Sub, so a restart or reconnect never misses the event), and otherwise sleeps until the next real deadline across hosts, archive profiles, and nightly maintenance instead of a fixed tick — no periodic safety-net ceiling: a due host with no entry (or an offline one) in the resolver cache is excluded from the wakeup computation entirely and waits on its own online event instead of being polled, while a host configured `noOnlineDetection: true` (e.g. fixed-IP hosts on a separate network) is exempt and still retried on a short backoff until found. No Apalis cron involved: each category (host, archive profile, nightly) tracks its own persisted "last done" state and is checked in the same unified loop. A config change only takes effect on scheduler restart. Honors per-host/global blackout windows. See [SERVER_COMPONENTS.md](SERVER_COMPONENTS.md#4-scheduler).
