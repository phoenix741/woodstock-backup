...
* A host that is **due but not currently known online** is not polled on a timer at all: it is excluded from the sleep computation entirely and relies solely on the online event above. This includes a host that has simply never self-registered recently — a self-registering host's online signal expires a couple of minutes after its last heartbeat, so "no recent signal" is the normal shape of "offline", not a rare corner case. A host that structurally can never self-register (typically a fixed-IP host on a separate/unreachable network) must set `noOnlineDetection: true` in its own `<hostname>.yml` (see [Host Configuration](./configuration.md)) — otherwise it would eventually stop being retried entirely, waiting for an online event it can never produce. With that flag set, it is retried periodically on a short backoff until it's found — see `retryBackoffOnRefusalSecs` below.
...
| retryBackoffOnRefusalSecs | `900` (15 min) | Cooldown recorded for a host after a refused scheduling attempt (already running, blocked by an active pool fsck lock, or unreachable while its reachability isn't otherwise tracked — e.g. a host with `noOnlineDetection: true`). A host not known online (no entry, or an explicit offline entry, in the resolver cache) is not retried on this backoff at all — it relies entirely on the online event, so this setting no longer governs it. |
