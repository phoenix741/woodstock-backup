...
import { bigIntTransformation } from '../utils';
...
export class QueueTaskProgression {
  ...
  get percent(): number {
    if (this.progressMax) {
      return Number((this.progressCurrent * 10_000n) / this.progressMax) / 100;
    }
    return 0;
  }
  ...
  get speed(): number {
    ...
    return Number((this.progressCurrent * 100n) / BigInt(duration)) / 100;
  }
  ...
  merge(progression?: QueueTaskProgression): QueueTaskProgression {
    ...
    this.modifiedCompressedFileSize += progression?.modifiedCompressedFileSize ?? 0n;

    this.fileSize += progression?.fileSize ?? 0n;
    this.newFileSize += progression?.newFileSize ?? 0n;
    this.modifiedFileSize += progression?.modifiedFileSize ?? 0n;

    this.newFileCount += progression?.newFileCount ?? 0;
    this.fileCount += progression?.fileCount ?? 0;
    this.removedFileCount += progression?.removedFileCount ?? 0;
    this.modifiedFileCount += progression?.modifiedFileCount ?? 0;

    this.errorCount += progression?.errorCount ?? 0;

    this.startDate = progression?.startDate ?? this.startDate;
    this.startTransferDate = progression?.startTransferDate ?? this.startTransferDate;
    this.endTransferDate = progression?.endTransferDate ?? this.endTransferDate;
...
export class QueueTasksInformations<GlobalContext> {
  constructor(
    public tasks: QueueTasks,
    public readonly context: QueueTaskContext<GlobalContext>,
  ) {}
}
