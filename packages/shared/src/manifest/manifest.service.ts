...
export class ManifestService {
  ...
  async writeJournalEntry<T = FileManifestJournalEntry>(
    ...
    return await writeAllMessages<FileManifestJournalEntry>(
  ...
  readManifestEntries(manifest: Manifest): AsyncIterableX<FileManifest> {
    return pipe(
      readAllMessages<FileManifest>(manifest.manifestPath, ProtoFileManifest),
  ...
  readJournalEntries(manifest: Manifest): AsyncIterableX<FileManifestJournalEntry> {
    return pipe(
      readAllMessages<FileManifestJournalEntry>(manifest.journalPath, ProtoFileManifestJournalEntry),
      map((frame) => frame.message),
      catchError((err) => {
  ...
  readFilelistEntries(manifest: Manifest): AsyncIterableX<FileManifestJournalEntry> {
    return pipe(
      readAllMessages<FileManifestJournalEntry>(manifest.fileListPath, ProtoFileManifestJournalEntry),
      map((frame) => frame.message),
      catchError((err) => {
        this.logger.warn(`Can't read the file ${manifest.journalPath}: ${err.message}`);
        return from([]);
  ...
  async compact<T>(
    ...
    await writeAllMessages(manifest.newPath, ProtoFileManifest, allMessages$.pipe(map(mapping))).then(async () => {
      try {
