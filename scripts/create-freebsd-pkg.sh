...
build_pkg() {
    ...
    # Generate the manifest (version substitution only)
    sed -e "s|@@VERSION@@|${VERSION}|g" \
        "${MANIFEST_IN}" > "${STAGING}/+MANIFEST"

    # Copy lifecycle scripts as separate files (pkg create picks them up automatically)
    install -m 755 "${SCRIPTS_DIR}/post-install.sh"  "${STAGING}/+POST_INSTALL"
    install -m 755 "${SCRIPTS_DIR}/pre-deinstall.sh" "${STAGING}/+PRE_DEINSTALL"
