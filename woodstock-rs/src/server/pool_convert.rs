...
use crate::{
    ...
    utils::{files::copy_files, lock::PoolLock},
...
impl PoolConvert {
    ...
    pub async fn convert_refcnt<P1: AsRef<Path>, P2: AsRef<Path>>(
        ...
        for unused in refcnt.list_unused() {
            let old_hash = &unused.sha256;
            let new_hash = chunks_map.get(old_hash);

            if let Some(new_hash) = new_hash {
                new_refcnt.add_unused(PoolUnused {
    ...
    pub async fn convert_chunk(
        ...
        callback: &impl Fn(&ConvertProgression),
    ) -> Result<usize> {
        ...
        for refcnt in chunks {
            ...
            callback(&ConvertProgression { progress_current });
        }
    ...
    async fn convert_manifest(
        ...
        // Traitement des entrées du manifeste
    ...
    pub async fn convert_backup_dir(
        ...
        let total_count = self
            .convert_chunk(new_config, &|progress| {
                callback(progress);
            })
            .await?;

        let () = self
            .convert_all_manifests(new_config, &|progress| {
                callback(&ConvertProgression {
                    progress_current: total_count + progress.progress_current,
                });
            })
            .await?;
